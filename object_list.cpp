...
std::shared_ptr<object> object_list::pop(std::vector<std::shared_ptr<object>  > :: iterator::difference_type i) {
...
std::ostream &object_list::print(std::ostream &a) const {
  a << '[' <<  *(list_.front());
  std::for_each(++list_.cbegin(), list_.cend(), [&](const std::shared_ptr<object> &b) -> void {
    a << ", " <<  *b;
  });
  a << ']';
  return a;
...
bool object_list::lt(const object &b) const {
  ...
  return std::lexicographical_compare(list_.cbegin(), list_.cend(), b_list.list_.cbegin(), b_list.list_.cend());
}
